"""3D artefact generator — SteelMemberGeometry -> model.glb + model.step.
...
Model space (metres): X = flange width, Z = overall section depth (welded
I-section), Y = cantilever length (member axis). The fabricated member is modelled
as a doubly-symmetric welded I-section extruded over its length. Fixed parametric
build123d template — never generated CAD. The solid verifies its own volume
against the closed-form section area x length before export.
"""
...
from components.structural_steel_member.params import SteelMemberGeometry
...
def analytic_steel_volume_m3(geometry: SteelMemberGeometry) -> float:
    ...
    length = geometry.cantilever_length_mm / MM_PER_M
    section_area = tw * dw + 2.0 * bf * tf
    return section_area * length


def _i_section_points(geometry: SteelMemberGeometry) -> list[tuple[float, float]]:
    bf = geometry.flange_width_mm / MM_PER_M
    tf = geometry.flange_thickness_mm / MM_PER_M
    tw = geometry.web_thickness_mm / MM_PER_M
...
def build_member_solid(geometry: SteelMemberGeometry) -> Part:
...
def _verify(solid: Part, geometry: SteelMemberGeometry) -> None:
...
def generate_solid(geometry: SteelMemberGeometry, out_dir: Path) -> dict[str, Path]:
