...
welded steel plate-girder superstructure design for Indian Railways. The engineering
...
limit, severity, detail) covering the design basis, the 25t live-load EUDL/CDA
re-derivation, bending-stress and web-shear adequacy, deflection, web slenderness and
stiffeners, fatigue, the independent section cross-check, and the calc-vs-drawing
read-back, plus any warnings.
...
3. **IRS Steel Bridge Code / IS 800 codes only.** Cite only the codes named in the
   FACTS block (IRS Steel Bridge Code, IS 800, IR Bridge Rules). Never cite concrete
   (IS 456), road-congress (IRC), or other national codes.
...
5. **Honesty stays.** If the FACTS note pending verification (transcribed permissible
   stresses, the transcribed 25t loading tables, the deferred fatigue assessment),
   reflect that honestly.
6. **Name the member.** When a strength non-conformity is present (e.g. bending on the
   flanges or shear in the web of an under-designed section), name the affected member
   and the clause.
