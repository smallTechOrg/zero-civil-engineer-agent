...
from components.rolling_stock_member.model3d import (
    analytic_steel_volume_m3,
    build_member_solid,
    generate_solid,
)
from components.rolling_stock_member.params import RollingStockMemberGeometry, RollingStockMemberParams
...
def test_solid_volume_matches_the_closed_form(geometry):
    ...
    assert box.Y == pytest.approx(geometry.member_length_mm / 1000.0, abs=1e-3)
...
def test_solid_builds_for_a_deep_long_member(tmp_path: Path):
    geometry = size_member(RollingStockMemberParams(member_length_m=12.0)).geometry
    solid = build_member_solid(geometry)
