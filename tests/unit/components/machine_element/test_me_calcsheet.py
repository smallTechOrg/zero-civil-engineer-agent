...
PARAMS = MachineElementParams(power_kw=20.0, speed_rpm=1000.0)
...
def test_calc_sheet_is_written_with_the_expected_sections(tmp_path: Path):
    ...
    assert section_ids == ["design_basis", "loading", "strength", "element_checks"]
...
def test_every_check_line_references_a_real_trail_step(tmp_path: Path):
    ...
    section_checks = next(s for s in doc["sections"] if s["id"] == "element_checks")
