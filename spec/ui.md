...
- **Component picker / gallery** (top of the left rail, Expansion Phase 1): a compact gallery of cards from `GET /api/components`, grouped by domain (Civil / Mechanical). **Available** cards (Box Culvert, Retaining Wall) are selectable and show the declared code set; **`coming_soon`** cards (plate girder, slab/T-beam, pier & abutment, and a Mechanical group) render greyed with a "Coming soon" badge — clickable only to show what's planned, never an error. Selecting a card sets the active component and switches the prompt-box placeholder + type-aware hints; submitting sends `component_type`. A "Let the agent decide" option leaves it to auto-detect.
...
- **Greyed component cards** (plate girder, slab/T-beam, pier & abutment, Mechanical): muted card + "Coming soon" badge + one line naming what it will design; clickable to reveal the roadmap, visually distinct from available cards (never an error/disabled-looking bug).
