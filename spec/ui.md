...
- **Component picker / gallery** (top of the left rail, Expansion Phase 1): a compact gallery of cards from `GET /api/components`, grouped by domain (Civil / Mechanical). **Available** cards — the Civil types (Box Culvert, Retaining Wall, Plate Girder, Slab / T-beam, Pier & Abutment, **M-00004 Standard Box Culvert (RDSO)**) and the three Mechanical types (Structural Steel / Fabrication Member, Rolling-Stock Member, Machine Element) — are selectable and show the declared code set. The picker still greys any **`coming_soon`** roadmap card (a "Coming soon" badge, clickable only to show what's planned, never an error), but the whole current roadmap is delivered, so **all cards are available and none are greyed**. Selecting a card sets the active component and switches the prompt-box placeholder + type-aware hints; submitting sends `component_type`. A "Let the agent decide" option leaves it to auto-detect.
- **Parameter form (params-direct components).** A **standard-driven** component (M-00004 Standard Box Culvert) is form-only: picking its card reveals `M00004ParamForm.tsx` in the left rail **in place of** the NL prompt box (the component declares no auto-detect — it is reachable only here). The form has typed, range-validated fields (clear span, clear height, cushion/fill, surcharge, formation width, side slope, concrete/steel grade) with the same defaults as `M00004Params`; **Design** submits the typed `params` object (`POST …/designs` with `component_type` + `params`). Client-side validation mirrors the server ranges so a bad value is caught before submit; a `422 PARAMS_INVALID` is surfaced inline against the offending field. Because intake is bypassed, the tracker's Understand + Extract steps appear immediately `done` ("entered via the parameter form") — never a stuck/pending step.
...
1. **Drawing** *(real from Phase 1)* — the GA drawing SVG inlined inside a pan/zoom wrapper (`react-zoom-pan-pinch`: wheel zoom, drag pan, double-click reset, zoom controls). Toolbar: **Download DXF** button (`ga.dxf` — "opens in AutoCAD and free viewers"). Loading state: skeleton sheet with "Drawing the GA…" while the run is mid-Draw. **M-00004 sheet (PDF):** when the run's artefacts include the `m00004_sheet` kind (M-00004 Standard Box Culvert), the toolbar also shows an **Open M-00004 sheet (PDF)** button that opens `m00004_sheet.pdf` inline in a new tab (the standard sheet: dimensioned cross-section, a1..h reinforcement in position, schedule table, notations, notes, title block — every catalogue value marked PROVISIONAL). The button is absent for components that don't emit the PDF (never a broken/empty control).
...
**Key files (frontend slice surface):** `frontend/src/app/page.tsx` (studio layout), `frontend/src/components/{ComponentPicker,M00004ParamForm,DetectedTypeChip,StepTracker,StatusLine,PromptPanel,TurnHistory,SuggestionChips,ArtefactTabs,TypeSummaryPanel,DrawingViewer,CalcSheet,ProofCheckPanel,Model3DViewer,LibraryPanel,PresetsEditor,TokenCostBadge}.tsx`, `frontend/src/lib/{api.ts,sse.ts,types.ts}`. `M00004ParamForm` (M-00004 component addition) is the typed parameter form shown in place of the prompt box when the M-00004 card is picked; it submits a `params` object. `TypeSummaryPanel` renders `component_type`-specific summaries (stability for the retaining wall) generically from `type_summary`.
