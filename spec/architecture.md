...
- **Explicit picker:** the frontend gallery lists every registered component (available + greyed `coming_soon`). Choosing one passes `component_type` on `POST /api/sessions/{id}/designs`; when present it **overrides** auto-detect (the LLM still validates scope and produces a type-aware plan/prompt hints).
...
> **Future mechanical libraries (NOTED, NOT ADDED now — add none until the mechanical phase lands):** a mechanical phase will likely need a weld-symbol / mechanical-drafting helper on top of `ezdxf` (GD&T + weld symbols are drawn from hand-validated parametric templates, same rule as civil — no new CAD lib unless `ezdxf` proves insufficient), and may reuse `anastruct` for steel-frame members (IS 800) and `build123d` for machine-element solids. RDSO/IS-800 code tables are transcribed data, not a dependency. No mechanical dependency is added in this expansion phase — the Component Registry keeps the plug-in surface, not the library set, as the extension point.
